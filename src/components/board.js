...
class Board extends Component {
	...
	componentDidMount() {
		...
		this.socket.on('playAgain', (board) => {
	...
	render() {
		...
		let tableBoard = this.state.board.map((row, index) => {
			let cols = row.map((col, colIndex) => {
				return (
					<td key={[index, colIndex]} onClick={this.handleClick.bind(null, index, colIndex)}>
						{this.state.board[index][colIndex]}
					</td>
				);
			});
		...
		if (this.state.resetGame) {
			btn = (
				<button className='btn btn-primary' onClick={this.startAnotherGame.bind(null)}>
					Rematch
				</button>
			);
		} else {
