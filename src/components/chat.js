    ...
    render() {
        ...
        let conversation = this.state.conversation.map((message, index) => {
            if (message.user == this.user) {
                ...
                return <div style={{textAlign: 'right'}}><p className="opponentMessage"
                                                            key={index}>{message.message}</p></div>;
            }
        });
        if (this.state.box) {
        ...
        if (this.state.btn) {
            btn =
                <button className="btn btn-info" onClick={this.openBox}>{this.state.messageAlert}{notification}</button>
        }
        return (
            <div>
